...
export async function generateMetadata({ params }: PageProps) {
  ...
  return {
    title: `Artikel dengan Tag: ${tag.name} - Blog CMS`,
  };
}
  ...
  return (
    <div className="min-h-screen bg-zinc-50 dark:bg-zinc-950 flex flex-col">
      ...
      <main className="flex-1 w-full max-w-6xl mx-auto px-6 py-12">
        <div className="mb-12">
          <p className="text-sm font-semibold text-blue-600 mb-2 uppercase tracking-wider">Menampilkan Tag</p>
          <h1 className="text-4xl font-extrabold tracking-tight text-zinc-900 dark:text-zinc-50">#{tag.name}</h1>
          <p className="text-zinc-600 dark:text-zinc-400 mt-2 text-lg">Ditemukan {tag.posts.length} artikel dengan tag ini.</p>
        </div>
